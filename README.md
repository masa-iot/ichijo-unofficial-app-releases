# ICHIJO施主手帳【非公式】

家づくりも、入居後の暮らしも。一条工務店の施主のためのサポートアプリ

**[アプリ紹介ページ](https://ichijo-unofficial.smartolio.jp/)** / **[ユーザーマニュアル](https://ichijo-unofficial.smartolio.jp/manual.html)** / **[ぐらんちゃん特設ページ](https://ichijo-unofficial.smartolio.jp/granchan.html)**

> キャラクター「ぐらんちゃん」のローマ字表記は `granchan`（dなし）。
> ページ運用の詳細・スタンプ新弾発売時の更新手順は
> `grandchan_stamp/引き継ぎ.md`（スタンプ制作リポジトリ）の「ぐらんちゃん特設ページ」の章を参照。

## ダウンロード

- Android: [Google Play](https://play.google.com/store/apps/details?id=jp.smartolio.ichijo_unofficial)
- iOS: [App Store](https://apps.apple.com/jp/app/ichijo%E6%96%BD%E4%B8%BB%E6%89%8B%E5%B8%B3-%E9%9D%9E%E5%85%AC%E5%BC%8F/id6761674540)

## 主な機能

- **ドキュメント管理** — 図面番号ベースで全書類を一元管理
- **図面チェック** — CAD・電気図面の2バージョン差分比較
- **見積り管理** — 自動パース・2版比較・推移表・CSVエクスポート
- **仕様書チェック** — 設備仕様書のセクション別差分
- **ウチメモ+** — 図面や見積りにその場でメモ、元の画面にワンタップで戻れる
- **でんき通信簿**（入居後） — 太陽光・蓄電池の経済効果を見える化
- **お手入れ当番表**（入居後） — 定期お手入れの消し込み管理・消耗品在庫・通知

## 注意事項

このアプリは一条工務店の公式アプリではありません。施主個人が開発した非公式アプリです。
